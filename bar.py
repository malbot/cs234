    ...
    def update(self, current, values=[], exact=[], strict=[], exp_avg=[]):
        ...
        if self.verbose == 1:
            ...
            if current < self.target:
                ...
                info += ' - {0:g}s'.format(now - self.start)
            for k in self.unique_values:
                if type(self.sum_values[k]) is list:
                    info += ' - {0}: {1:.4f}'.format(k, self.sum_values[k][0] / max(1, self.sum_values[k][1]))
                else:
                    info += ' - {0}: {1}'.format(k, self.sum_values[k])

            for k, v in self.exp_avg.items():
                info += ' - {0}: {1:.4f}'.format(k, v)
            ...
            sys.stdout.flush()
            # print("{0}{1}".format(bar, info), end='\r')
        ...
        if self.verbose == 2:
            if current >= self.target:
                info = ' - {0:d}s'.format(now - self.start)
                for k in self.unique_values:
                    info += ' - {0}: {1:.4f}'.format(k, self.sum_values[k][0] / max(1, self.sum_values[k][1]))
                sys.stdout.write(info + "\n")
