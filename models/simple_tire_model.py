...
class SimpleTireModel(object):
    ...
    def __call__(self, Fz, alpha, as_dict=False):
        """
        ...
        """
        alpha_sl = math.atan2(3*self.mu_p*Fz, self.cy)
        if abs(alpha) > alpha_sl:
            fy = -self.mu_s*Fz*(np.sign(alpha))
        else:
            fy = -self.cy*math.tan(alpha) \
                 + self.cy**2/(3*self.mu_p*Fz)*(2 - self.mu_s/self.mu_p)*math.tan(alpha)*abs(math.tan(alpha)) \
                 - self.cy**3/((3*self.mu_p*Fz)**2)*(math.tan(alpha)**3)*(1-((2*self.mu_s)/(3*self.mu_p)))
        return fy
