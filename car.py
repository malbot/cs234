...
class CarModel():
    ...
    def __call__(self, state, action, time, as_state=True):
        ...
        dsdt = Ux * np.cos(state.delta_psi) - Uy * np.sin(state.delta_psi)
        dedt = Uy * np.cos(state.delta_psi) + Ux * np.sin(state.delta_psi)
        s = dsdt * time + state.s
        e = dedt * time + state.e
        dDpdt = r - dsdt * state.kappa(s)[0]
        delta_psi = dDpdt * time + state.delta_psi
